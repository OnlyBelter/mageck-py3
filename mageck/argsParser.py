...
def arg_test(subparser):
    ...
    gengroup.add_argument('--paired',action='store_true',help='Paired sample comparisons. In this mode, the number of samples in -t and -c must match and have an exact order in terms of samples. For example, "-t HL60.final,KBM7.final -c HL60.initial,KBM7.initial".')
    ...
    cnvcorgroup.add_argument('--cell-line',help='The name of the cell line to be used for copy number variation normalization.')
...
def arg_mle(subparser):
    ...
    mlegroup.add_argument('--max-sgrnapergene-permutation',type=int,default=100,help='Do not calculate beta scores or p vales if the number of sgRNAs per gene is greater than this number. This will save a lot of time if some isolated regions are targeted by a large number of sgRNAs (usually hundreds). Must be an integer. Default 100.')
