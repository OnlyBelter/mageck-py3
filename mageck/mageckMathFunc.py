...
def getVars(matt):
    ...
    varvalue = [
        sum([(kj - meanvalue[i]) * (kj - meanvalue[i]) for kj in matt[i]])
        / max(float(len(matt[i])) - 1, 1.0)
  ...
  """
    ...
    def __init__(self):
        ...
        try:
            ...
            logging.error(
                "Cannot open qnorm_table.txt in the default mageck source folder: "
                + qnormfile
                + " for QC. Please double check your file location and format."
            )
            sys.exit(-1)
    ...
    """
        ...
        frac_value = [
            (lp[i] - self.logp_vec[ind_min[i]]) / self.qstep for i in range(len(lp))
        ]
        frac_value = [min(max(x, 0.0), 1.0) for x in frac_value]
        interp_value = [
            self.norm_qnorm_vec[ind_min[i]] * (1.0 - frac_value[i])
            + self.norm_qnorm_vec[ind_min[i] + 1] * frac_value[i]
            for i in range(len(lp))
        ]
        interp_value_ret = [
            interp_value[i] if (lp[i] >= self.qmin and lp[i] <= self.qmax) else None
            for i in range(len(lp))
        ]
        if type(p) is not list:
