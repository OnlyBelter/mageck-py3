...
# from mageck.mleargparse import *
...
def mageckmle_main(pvargs=None,parsedargs=None,returndict=False):
  ...
  if args.cnv_est is not None:
    ...
    mageckmleCNVestimation(args.cnv_est,cttab_sel,desmat,sgrna2genelist,args.beta_labels[1:],args.output_prefix)
  ...
  if args.cnv_norm is not None or args.cnv_est is not None: 
    if args.cnv_norm is not None: # get copy number data from external copy number dataset
      ...
      logging.info('Performing copy number normalization using copy number estimates.')
      (CN_arr,CN_celldict,CN_genedict) = read_CNVdata(str(args.output_prefix)+'CNVestimates.txt',args.beta_labels[1:])
    ...
    betascore_piecewisenorm(allgenedict,args.beta_labels,CN_arr,CN_celldict,CN_genedict,selectGenes=genes2correct)
