...
def mageckcount_printstat(args,datastat):
  ...
  n=len(datastat)
  crv.insertReadCountBoxPlot(os.path.basename(outcsvfile),nsample=n)
  if len(alllabels) > 1:
    crv.insertPCAPlot(os.path.basename(outcsvfile),nsample=n)
    crv.insertClusteringPlot(os.path.basename(outcsvfile),nsample=n)
  crv.closeRTemplate()
