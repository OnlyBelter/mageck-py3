...
def modelmeanvar(ctable, method="edger"):
    ...
    if len(meanvalue) > 10:
        ...
        logging.info(
            "Detecting outliers from variance estimation.. Avg read count:"
            + str(m_mean)
            + ", Var: "
            + str(v_mean)
        )
    ...
    if ignore_points > 0:
        logging.info(
            "Skipping "
            + str(ignore_points)
            + " sgRNAs from variance calculation because of their extreme high counts (> 4* STD (mean counts) )."
        )
    # choose values with variance greater than mean
    meangood = [
        meanvalue[i]
        for i in range(len(meanvalue))
        if meanvalue[i] < varvalue[i] and meanvalue[i] < high_cutoff
    ]
    vargood = [
        varvalue[i] - meanvalue[i]
        for i in range(len(varvalue))
        if meanvalue[i] < varvalue[i] and meanvalue[i] < high_cutoff
    ]
    if len(meangood) < 10:
        logging.warning(
            "The variances between control replicates are too small. If they are technical replicates, merge these into one sample."
        )
        meangood = [meanvalue[i] for i in range(len(meanvalue))]
        vargood = [
            (lambda x: x if x > 0.01 else 0.01)(varvalue[i] - meanvalue[i])
            for i in range(len(varvalue))
        ]
    # log
...
def getadjustvar(coef, meanval, method="mixed"):
    ...
    if method == "linear":
        ...
        return [
            (lambda x, y: x if x > y else y)(var1[i], var2[i]) for i in range(len(var1))
        ]
    else:
        return meanval


def calculate_gene_lfc(
    args,
    lfcval,
    sort_id,
    n_lower,
...
def crispr_test(tab, ctrlg, testg, varg, destfile, sgrna2genelist, args):
  ...
  """
    ...
    # [writer.writerow([k,v]) for (k,v) in sgrna2genelist.iteritems()]
    # f.close()
    ...
    if args.cnv_est is not None:
        ...
        mageckCNVestimation(
            args.cnv_est, tabctrl, tabtest, sgrna2genelist, "CNVest", args.output_prefix
        )
    #
    #
    #
    ...
    if len(varg) > 1:
        tabctrlmod = {
            k: [v[i] for i in range(len(v)) if i in varg] for (k, v) in tab.items()
        }
    # elif len(ctrlg)>1 and args.variance_from_all_samples==False: # more than 1 controls
    elif len(ctrlg) > 1:  # more than 1 controls
        tabctrlmod = {
            k: [v[i] for i in range(len(v)) if i in ctrlg] for (k, v) in tab.items()
        }
    ...
    # treatment matrix
    tabtreatmod = {
        k: [v[i] for i in range(len(v)) if i in (testg)] for (k, v) in tab.items()
    }
    ...
    if hasattr(args, "remove_zero"):
        validsgrna1 = [
            (lambda x: 1 if x > args.remove_zero_threshold else 0)(t) for t in tabc_mean
        ]
    ...
    if False:
        ...
        n_test = len(testg)
        if (
            len(varg) > 1 or len(ctrlg) > 1
        ):  # more than 1 controls, or users specify that variance should be calculated from certain samples
    ...
    # nt = tabtest[list(tabtest.keys())[0]]
    ...
    if hasattr(args, "remove_zero"):
        validsgrna2 = [
            (lambda x: 1 if x > args.remove_zero_threshold else 0)(t) for t in ttmean
        ]
        if args.remove_zero == "control":
            ...
            validsgrna = [
                1 - (1 - validsgrna1[t]) * (1 - validsgrna2[t]) for t in range(n)
            ]
        else:
            validsgrna = [1] * n
    else:
    ...
    if False:
        ...
        tt_theta = [
            (ttmean[i] - tabc_mean[i]) / math.sqrt(tabc_adjvar[i]) for i in range(n)
        ]
    if False:
        ...
        tt_theta = [
            (
                ...
                if ttmean[i] < tabc_mean[i]
                else -1 * tt_p_higher_log[i]
            )(x)
            for x in range(n)
        ]
    if True:
        ...
        tt_theta_low = [
            tt_theta_low_cvt[i] if tt_theta_low_cvt[i] is not None else tt_theta_0[i]
            for i in range(n)
        ]
        tt_theta = [
            (lambda i: tt_theta_low[i] if ttmean[i] < tabc_mean[i] else tt_theta_0[i])(
                x
            )
            for x in range(n)
        ]
    ...
    #
    tt_p_twosided = [
        (lambda x, y: 2 * x if x < y else 2 * y)(tt_p_lower[i], tt_p_higher[i])
        for i in range(n)
    ]
    ...
    CNVnorm = False
    if (
        args.cnv_norm is not None and args.cell_line is not None
    ) or args.cnv_est is not None:
        from mageck.cnv_normalization import (
            read_CNVdata,
            sgRNAscore_piecewisenorm,
            highestCNVgenes,
        )
        ...
        if args.cell_line in CN_celldict or "CNVest" in CN_celldict:
            if args.cell_line in CN_celldict:
                logging.info(
                    "Normalizing by copy number with "
                    + args.cell_line
            ...
            CNVnorm = True
            norm_tt_theta = sgRNAscore_piecewisenorm(
                tt_theta, gene_list, CN_arr, CN_genedict, selectGenes=genes2correct
            )
            norm_tt_abstheta = [math.fabs(norm_tt_theta[i]) for i in range(n)]
            sort_id = [
                i[0]
                for i in sorted(
                    enumerate(norm_tt_abstheta), key=lambda x: x[1], reverse=True
                )
            ]
        ...
        sort_id = [
            i[0]
            for i in sorted(enumerate(tt_abstheta), key=lambda x: x[1], reverse=True)
        ]
    #
    #
    #
    ...
    sgrnalfc = [
        math.log(ttmean[i] + 1.0, 2) - math.log(tabc_mean[i] + 1.0, 2) for i in range(n)
    ]
    #
    #
    #
    ...
    if sgrna2genelist is not None:
        ...
        destf = open(destfname, "w")
        print(
            "\t".join(["sgrna", "symbol", "pool", "p.low", "prob", "chosen"]),
            file=destf,
        )
        for i in sort_id:
            report = [
                destkeys[i],
                sgrna2genelist[destkeys[i]],
                "list",
                tt_p_lower_score[i],
                "1",
                validsgrna[i],
        ...
        n_lower_valid = sum(
            [
                1
                for n_i in range(n)
                if (tt_p_lower[n_i] <= args.gene_test_fdr_threshold)
                and (validsgrna[n_i] == 1)
            ]
        )
        ...
        # calculate gene lfc
        lower_gene_lfc = calculate_gene_lfc(
            args,
            sgrnalfc,
            sort_id,
            n_lower,
        ...
        sort_id = [
            i[0]
            for i in sorted(
                enumerate(tt_p_higher_score), key=lambda x: x[1], reverse=False
            )
        ]
        # output to file
        print(
            "\t".join(["sgrna", "symbol", "pool", "p.high", "prob", "chosen"]),
            file=destf,
        )
        for i in sort_id:
            report = [
        ...
        n_higher_valid = sum(
            [
                1
                for n_i in range(n)
                if (tt_p_higher[n_i] <= args.gene_test_fdr_threshold)
                and (validsgrna[n_i] == 1)
            ]
        )
        if n_higher > 0:
        ...
        # calculate gene lfc
        higher_gene_lfc = calculate_gene_lfc(
            args,
            sgrnalfc,
            sort_id,
            n_higher,
...
def rank_association_test(file, outfile, cutoff, args, adjustcutoff=True):
    ...
    if (
        hasattr(args, "additional_rra_parameters")
        and args.additional_rra_parameters != None
    ):
    ...
    for dl in args.day0_label:
        if dl not in samples:
            logging.error(
                "Label "
                + dl
                + " specified in the --day0-label option does not match count table. Please double check."
            )
            sys.exit(-1)
...
def magecktest_main(args):
  ...
  """
    ...
    if len(cttab) == 0:
        sys.exit(-1)
    nsample = len(cttab[list(cttab.keys())[0]])
    ...
    vrv.startRTemplate()
    vrvrnwcplabel = []
    # labels to write in rnw

    # if users specify variance estimation samples
    if args.variance_estimation_samples is not None:
        (vargroup, vargrouplabellist) = parse_sampleids(
            args.variance_estimation_samples, samplelabelindex
        )
        logging.info(
            "Use the following samples for variance estimation:"
            + ",".join(vargrouplabellist)
        )
        if len(vargroup) < 2:
    ...
    for cpindex in range(len(supergroup_treat)):
        ...
        # labels
        (treatgroup, treatgrouplabellist) = parse_sampleids(
            supergroup_treat[cpindex], samplelabelindex
        )
        ...
        if supergroup_control != None:
            (controlgroup, controlgrouplabellist) = parse_sampleids(
                supergroup_control[cpindex], samplelabelindex
            )
            ...
            (controlgroup, controlgrouplabellist) = parse_sampleids(
                ",".join([str(t) for t in xls]), samplelabelindex
            )
        ...
        # normalization
        cttab_sel = {
            k: ([v[i] for i in selected_samples_id]) for (k, v) in cttab.items()
        }
        # controlgroup do not overlap with treatgroup
        norm_msg = NormMsgs()
        if hasattr(args, "norm_method"):
            nttab = normalizeCounts(
                cttab_sel,
        ...
        if hasattr(args, "normcounts_to_file") and args.normcounts_to_file:
            # counts
            mageck_printdict(
                nttab, args, sgrna2genelist, samplelabelindex, controlgroup + treatgroup
            )
        ...
        controlgroup_ids = [
            i
            for i in range(len(selected_samples_id))
            if selected_samples_id[i] in controlgroup
        ]
        treatgroup_ids = [
            i
            for i in range(len(selected_samples_id))
            if selected_samples_id[i] in treatgroup
        ]
        vargroup_ids = [
            i
            for i in range(len(selected_samples_id))
            if selected_samples_id[i] in vargroup
        ]
        #
        #
        # for paired samples
        if hasattr(args, "paired") and args.paired:
            ...
            for (k, vi) in nttab.items():
                for ci in range(len(controlgroup_ids)):
                    ...
                    vi_count += [
                        vi[s] for s in vargroup_ids
                    ]  # if var group is specified
        ...
        if gene_as_cutoff[0] is not None:
            rank_association_test(
                cp_prefix + ".plow.txt",
                cp_prefix + ".gene.low.txt",
                gene_as_cutoff[0],
                args,
            )
        if gene_as_cutoff[1] is not None:
            rank_association_test(
                cp_prefix + ".phigh.txt",
                cp_prefix + ".gene.high.txt",
                gene_as_cutoff[1],
                args,
                adjustcutoff=False,
            )  # update: fpr positive selection, do not adjust alpha cutoff
        # merge different files
        merge_rank_files(
            cp_prefix + ".gene.low.txt",
            cp_prefix + ".gene.high.txt",
            cp_prefix + ".gene_summary.txt",
            args,
            gene_as_cutoff,
        )
        if cpindex > 0:
            ...
            merge_rank_summary_files(
                args.output_prefix + ".gene_summary.txt",
                cp_prefix + ".gene_summary.txt",
                args.output_prefix + ".gene_summary.txt",
                args,
                lowfile_prefix=label1,
                highfile_prefix=label2,
            )
        ...
        vrv.loadTopKWithExp(
            ...
            nttab,
            sgrna2genelist,
            controlgrouplabellist + treatgrouplabellist,
        )
    ...
    vrv.comparisonlabel = vrvrnwcplabel
    ...
    if warning_in_norm:
        logging.warning(
            "Warning messages generated during normalization. Please double check with the log file to make sure the normalization process is proper."
        )
